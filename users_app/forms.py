...
class UserForm(UserCreationForm):
    ...
    user_birth = forms.DateField(label="생년월일", widget=forms.widgets.DateInput(attrs={'type':'date', 'max': date.today().strftime('%Y-%m-%d')}), required=True)
    user_gender = forms.ChoiceField(label="성별", choices=(("0", "남자"), ("1", "여자")), widget=forms.RadioSelect)
    user_height = forms.IntegerField(label="키", max_value=300, widget=forms.TextInput(attrs={'placeholder': '키 cm'}))
    user_weight = forms.IntegerField(label="몸무게", max_value=300, widget=forms.TextInput(attrs={'placeholder': '몸무게 kg'}))
    user_activity = forms.ChoiceField(label="활동수준", choices=(("5", "매우 활발한 활동"),("4","활발한 활동"),("3","보통 활동"),("2","약간의 활동"), ("1","거의 활동이 없음")))
    user_target_weight = forms.IntegerField(label="목표몸무게", max_value=300, widget=forms.TextInput(attrs={'placeholder': '몸무게 kg'}))
